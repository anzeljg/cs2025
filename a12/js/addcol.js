function addcolMain() {
    ...
    var s = "";
    s += '<div style="position:relative; width:' + w + 'px; min-height:' + h + 'px; border: none; border-radius: 20px; background-color: transparent; margin:auto; display:block;">';
    ...
    s += '<div id="plus" style="position:absolute; left:55px; top:45px; font:20px sans-serif; color:#808080;">+</div>';
    s += '<div id="lines" style="position:absolute; left:55px; top:70px; width:' + (colMax * 15 + 25) + 'px; height:27px; border-top:1px solid #808080;"></div>';
    s += '<div id="bad" style="position:absolute; left:150px; top:65px; font:bold 22px sans-serif; color:red;">(Error)</div>';
...
Col.prototype.getHTML = function() {
    var onestext = 'ones'; // ones
	var tenstext = 'tens'; // tens
	if (this.base == 'bin') tenstext = 'twos'; // twos
    var id = 'c' + this.n;
    var s = '';
    var hiClr = 'rgba(0,128,255,0.1)';
    //var hiClr2 = 'rgba(230,230,255,1)';
    var hiClr2 = 'rgba(0,128,255,0.1)';
    ...
    s += '</div>';
    s += '<div id="' + id + 'lblten" style="position:absolute; left:115px; top:45px; text-align:center; width:40px; font:12px sans-serif; color:#808080;' + std + '">' + tenstext + '</div>';
    s += '<div id="' + id + 'lblone" style="position:absolute; left:145px; top:45px; text-align:center; width:40px; font:12px sans-serif; color:#808080;' + std + '">' + onestext + '</div>';
...
function getPlayHTML(w) {
    var s = '';
    s += '<button id="playBtn" class="btn btn-primary" onclick="togglePlay()"><i id="btnIcon" class="fa fa-play"></i></button>';
    return s;
}